...
void recOurs(int d, vu pc0, vu pc1, vu v0, vu v1)
{
    int leaves = pow(2, d-1);
    vu res(leaves);
    for(int i = 0; i < pc0.size(); i++)
...
void clientFeatureEnc(vector<pair<int, int> > params)
{
    for(int i = 0; i < params.size(); i++)
    {
        ...
        cout << "SEDT," << params[i].first << "," << params[i].second << "," << temp_enc / TOTAL <<"," << temp_dec / TOTAL << endl;
        ...
        cout << "OSDT," << params[i].first << "," << params[i].second << "," << temp_enc / TOTAL <<"," << temp_dec / TOTAL << endl;

        temp_enc = 0, temp_dec = 0;
        for(int j = 0; j < TOTAL; j++)
        {
            temp_enc += measureTime(clientFeatureEncOurs, features, masks);
            temp_dec += measureTime(recOurs, params[i].first, pc0, pc1, v0, v1);
        }
