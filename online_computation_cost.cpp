...
{
    ull a1=14490808261858112199UL,a2=2177743953316042629UL;
    ull b1=12415856028556828342UL,b2=16338301252824554741UL;
    ull c1=14315882575126838720UL,c2=18193727490463377156UL;
    ull e1 = x * a1;
    ull f1 = y * b1;
    //get e2 f2
    ull e2 = 13587943239643466562UL;
    ull f2 = 10553232343782334634UL;
...
void testDecisionNodeEvl(vector<pair<int, int> > params)
{
    ...
    for(int i = 0; i < params.size(); i++)
    {
        ...
        cout << "z22,"<< std::fixed<<params[i].first << ","<<params[i].second << "," << temp <<", "
        << (ull)((2+3*(64-2))*trans_delay + (2+3*(64-2))*2/bandwidth) *  dn + temp<< endl;
        temp = measureTime(z23DecisionNodeEvl,yj, xsigm);
        cout << "z23," << params[i].first << ","<<params[i].second << ","<< temp <<", "<< 
        (ull)(7*trans_delay + 63*2*2/bandwidth) *  dn + temp << endl;
        
        temp = measureTime(oursDecisionNodeEvl, make_pair(head, tail), x);
        // oursDecisionNodeEvl(make_pair(head, tail), x);
        cout << "ours," <<params[i].first << ","<<params[i].second << ","<< temp <<", "<< temp *  dn << endl;
    }
